...
CREATE TABLE PARTIDO (
    codigo_partido INT PRIMARY KEY,                -- Código del partido (clave primaria)
    fecha DATE NOT NULL,                           -- Fecha del partido
    codigo_equipo_local INT NOT NULL,              -- Equipo local (clave foránea)
    codigo_equipo_visitante INT NOT NULL,          -- Equipo visitante (clave foránea)
    CONSTRAINT FK_Partido_EquipoLocal
        FOREIGN KEY (codigo_equipo_local)
        REFERENCES EQUIPO(codigo_equipo)
        ON DELETE CASCADE                          -- Si se elimina el equipo, se eliminan los partidos
        ON UPDATE CASCADE,
...
-- ============================================
CREATE TABLE GOL (
    codigo_partido INT NOT NULL,                   -- Código del partido (parte de la PK, FK)
...
INSERT INTO PARTIDO (codigo_partido, fecha, codigo_equipo_local, codigo_equipo_visitante)
...
SELECT P.codigo_partido, P.fecha, EL.nombre AS equipo_local, EV.nombre AS equipo_visitante
