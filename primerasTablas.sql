...
CREATE TABLE PRESIDENTE (
 dni CHAR(9) PRIMARY KEY, -- DNI del presidente (clave
primaria)
 ...
 fecha_nacimiento DATE NOT NULL -- Fecha de nacimiento del
presidente
);
-- ============================================
-- 2. EQUIPO
...
-- ============================================
CREATE TABLE EQUIPO (
 codigo_equipo INT PRIMARY KEY, -- Código único del equipo (clave
primaria)
 ...
 dni_presidente CHAR(9) UNIQUE, -- DNI del presidente (clave
foránea única)
 CONSTRAINT FK_Equipo_Presidente
 FOREIGN KEY (dni_presidente) -- FK hacia PRESIDENTE
 REFERENCES PRESIDENTE(dni)
 ON DELETE SET NULL -- Si se elimina el presidente,
se pone en NULL
 ON UPDATE CASCADE -- Si cambia el DNI, se actualiza
en EQUIPO
);
-- ============================================
-- 3. PARTIDO
...
-- ============================================
-- TABLA PARTIDO INCORRECTA
CREATE TABLE PARTIDO (
 codigo_partido INT PRIMARY KEY, -- Código del partido (clave
primaria)
 fecha DATE NOT NULL, -- Fecha del partido
 codigo_equipo_local INT NOT NULL, -- Equipo local (clave foránea)
 codigo_equipo_visitante INT NOT NULL, -- Equipo visitante (clave
foránea)
 CONSTRAINT FK_Partido_EquipoLocal
 FOREIGN KEY (codigo_equipo_local)
 REFERENCES EQUIPO(codigo_equipo)
 ON DELETE CASCADE -- Si se elimina el equipo, se
...
CREATE TABLE JUGADOR (
 codigo_jugador INT PRIMARY KEY, -- Código único del jugador
(clave primaria)
 nombre VARCHAR(100) NOT NULL, -- Nombre del jugador
 posicion VARCHAR(50) NOT NULL, -- Posición del jugador (ej:
defensa, delantero)
 fecha_nacimiento DATE NOT NULL -- Fecha de nacimiento del
jugador
);
...
-- ============================================
CREATE TABLE GOL (
 codigo_partido INT NOT NULL, -- Código del partido (parte de
la PK, FK)
 codigo_jugador INT NOT NULL, -- Código del jugador (parte de
la PK, FK)
 minuto INT NOT NULL, -- Minuto en que se marcó el gol
(parte de la PK)
 descripcion VARCHAR(255), -- Descripción del gol (opcional)
 CONSTRAINT PK_Gol PRIMARY KEY (codigo_partido, codigo_jugador, minuto), -- PK
compuesta
 CONSTRAINT FK_Gol_Partido
 FOREIGN KEY (codigo_partido)
 REFERENCES PARTIDO(codigo_partido)
 ON DELETE CASCADE -- Si se borra el partido, se
borran sus goles
 ON UPDATE CASCADE,
 CONSTRAINT FK_Gol_Jugador
 FOREIGN KEY (codigo_jugador)
 REFERENCES JUGADOR(codigo_jugador)
 ON DELETE CASCADE -- Si se borra el jugador, se
borran sus goles
 ON UPDATE CASCADE
);
-- ======== INSERT
...
WHERE dni = '23456789B';
-- Eliminar un equipo (también se eliminarán los partidos donde era local o
visitante)
DELETE FROM EQUIPO
WHERE codigo_equipo = 1;
